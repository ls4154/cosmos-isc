...
{
    printf("\t\t\tNAND_InitChCtlReg - Start\n");
    NAND_InitChCtlReg();
    printf("\t\t\tNAND_InitChCtlReg - Finish\n");

    printf("\t\t\tNAND_InitNandArray - Start\n");
    NAND_InitNandArray();
    printf("\t\t\tNAND_InitNandArray - Finish\n");

    g_pstNAND = (NAND_GLOBAL*)malloc(sizeof(NAND_GLOBAL));
    //g_pstNAND = (NAND_GLOBAL*)malloc(sizeof(NAND_GLOBAL));
    g_pstNAND->pstNAND_Completion = (NAND_COMPLETION*)dma_malloc(sizeof(NAND_COMPLETION), 0);
    g_pstNAND->pstNAND_Status = (NAND_STATUS*)dma_malloc(sizeof(NAND_STATUS), 0);
    g_pstNAND->pstNAND_Error = (NAND_ERROR*)dma_malloc(sizeof(NAND_ERROR), 0);
    ...
    for (int nCh = 0; nCh < USER_CHANNELS; nCh++)
    {
        for (int nWay = 0; nWay < USER_WAYS; nWay++)
        {
            ...
            g_pstNAND->stWayStatus[nCh][nWay].nRetry = 0;
        }
    }
}
...
{
    //cprint(NAND_DEBUG,  "[NAND] Erase - Ch/Way/Block:%d/%d/%d\r\n", nPCh, nPWay, nBlock);
    ...
    do
    {
        switch (*pnCurStatus)
        {
            ...
            case NAND_ERASE_STEP_WAIT_ERASE_DONE:
                if (_IsCMDIssuable(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG, "Ch(%d)/Way(%d)/Block(%d) Busy - Erase Done waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
            ...
            case NAND_ERASE_STEP_WAIT_GET_STATUS_DONE:
                ...
                {
                    //cprint(NAND_DEBUG,  "Ch(%d)/Way(%d)/Block(%d) Busy - Erase Get Status waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
...
{
    cprint(NAND_DEBUG,  "[NAND] Read - Ch/Way/Block/Page/bECCOn:%d/%d/%d/%d/%d\r\n", nPCh, nPWay, nBlock, nPage, bECCOn);
    ...
    do
    {
        switch (*pnCurStatus)
        {
            ...
            case NAND_READ_STEP_WAIT_READ_DONE:
                ...
                if (_IsCMDIssuable(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG, "Ch(%d)/Way(%d)/Block(%d) Busy - Read Trigger Done waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
            ...
            case NAND_READ_STEP_WAIT_GET_STATUS_READ_DONE:
                ...
                if (_IsWayIdle(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG,  "Ch(%d)/Way(%d)/Block(%d) Busy - Read Get Status waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
            ...
            case NAND_READ_STEP_WAIT_TRASFER_DONE:
                if (_IsCMDIssuable(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG, "Ch(%d)/Way(%d)/Block(%d) Busy - Read Transfer Done waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
...
{
    //cprint(NAND_DEBUG,  "[NAND] Program - Ch/Way/Block/Page:%d/%d/%d/%d\r\n", nPCh, nPWay, nBlock, nPage);
    ...
    do
    {
        switch (*pnCurStatus)
        {
            ...
            case NAND_PGM_STEP_CHECK_PROGRAM_ISSUABLE:
                if (_IsCMDIssuable(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG,  "Ch(%d)/Way(%d)/Block(%d) Busy - Program Issue waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
            ...
            case NAND_PGM_STEP_WAIT_PROGRAM_DONE:
                ...
                if (_IsCMDIssuable(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG,  "Ch(%d)/Way(%d)/Block(%d) Busy - Program Done waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
            ...
            case NAND_PGM_STEP_WAIT_GET_STATUS_DONE:
                ...
                if (_IsWayIdle(nPCh, nPWay) == FALSE)
                {
                    //cprint(NAND_DEBUG,  "Ch(%d)/Way(%d)/Block(%d) Busy - Program Get Status waiting\r\n", nPCh, nPWay, nBlock);
                    break;
                }
...
void NAND_InitNandArray(void)
{
    for (int nCh = 0; nCh < USER_CHANNELS; nCh++)
    {
        for (int nWay = 0; nWay < USER_WAYS; nWay++)
        {
            ...
            V2FEnterToggleMode(chCtlReg[nCh], nWay, (unsigned int)base_DMA);

            //printf("Reest & Toggle Set Complete @ Ch:%d / Way:%d\n", nCh, nWay);
        }
    }
}

void NAND_AllBlockErase(void)
{
    printf("[NAND_AllBlockErase] All Block Erase Start.\n");

    for (int nCh = 0; nCh < USER_CHANNELS; ++nCh)
    {
        for (int nWay = 0; nWay < USER_WAYS; ++nWay)
        {
            for (int nBlock = 0; nBlock < TOTAL_BLOCKS_PER_DIE; ++nBlock)
            {
                NAND_IssueErase(nCh, nWay, nBlock, 1);

                //printf("Erase Finish Ch: %d / Way: %d / Block: %d\n", nCh, nWay, nBlock);
            }
        }
    }

    printf("[NAND_AllBlockErase] All Block Erase Finish.\n");
}
